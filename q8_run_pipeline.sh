...
jupyter nbconvert --execute --to notebook --inplace q4_exploration.ipynb || { echo "Q4 failed" >> reports/pipeline_log.txt; exit 1; }
echo "Q4 completed" >> reports/pipeline_log.txt

jupyter nbconvert --execute --to notebook --inplace q5_missing_data.ipynb || { echo "Q5 failed" >> reports/pipeline_log.txt; exit 1; }
echo "Q5 completed" >> reports/pipeline_log.txt

jupyter nbconvert --execute --to notebook --inplace q6_transformation.ipynb || { echo "Q6 failed" >> reports/pipeline_log.txt; exit 1; }
echo "Q6 completed" >> reports/pipeline_log.txt

jupyter nbconvert --execute --to notebook --inplace q7_groupby.ipynb || { echo "Q7 failed" >> reports/pipeline_log.txt; exit 1; }
echo "Q7 completed" >> reports/pipeline_log.txt
